import { IonCard, IonCardHeader, IonToolbar, IonTitle, IonButton, IonIcon, IonCardContent, IonList, IonLabel, useIonAlert } from "@ionic/react"
    ...
    return(
        ...
        <IonList className="projectList">
          {deviceListContext.list?.map((item: DeviceInterface, index:number) => (
            <DeviceComponent data={item} key={item.id} index={index}></DeviceComponent>
          ))}
        </IonList>
      ) : (
